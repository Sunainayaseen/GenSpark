...
# ---------------------------------------------------------------------------
# MySQL — local .env (DB_*) or Railway (MYSQL_* / MYSQLHOST)
# ---------------------------------------------------------------------------
...
def _resolve_model_class_names(model) -> dict:
    """Prefer the trained model's own class map (best.pt); fall back to CLASS_NAMES.

    Using model.names keeps labels correct even if the model is retrained with a
    different class order — the old hardcoded {0:mouse,...} silently mislabels then.
    """
...
def _component_name(class_id: int, names: dict | None = None) -> str:
    table = names or CLASS_NAMES
    return table.get(class_id, CLASS_NAMES.get(class_id, f'class_{class_id}'))


def _boxes_to_detections(result, img_w: int, img_h: int, names: dict | None = None) -> list[dict]:
...
def _run_yolo_on_image(image, confidence: float = 0.35) -> tuple[dict | None, str | None]:
    ...
    try:
        ...
        detections = _boxes_to_detections(results[0], img_w, img_h, names)
        return {
...
# ---------------------------------------------------------------------------
# Catalog resolution — map a YOLO detection to real, in-stock catalog rows
# ---------------------------------------------------------------------------
# Detected class label (lowercase) -> component_categories.name in genspark_erp.
# Matching by category NAME (not id) is deliberate: the catalog has duplicate
# categories (e.g. 'Mouse' at id 14 and 18) and _search_local_components filters
# on cat.name, so a name match resolves rows under either duplicate.
...
DETECTION_MATCH_LIMIT = max(1, int(os.getenv('GENSPARK_DETECT_MATCH_LIMIT', '6')))

# Name keywords used to rank rows *within* a category. The catalog has mixed-up
# category assignments (e.g. cooling fans filed under 'RAM'), so a row whose name
# actually mentions the part is a better match than one that only shares a category.
...
def _keep_addable(cursor, rows: list[dict]) -> list[dict]:
    """Drop rows with no approved vendor in stock.

    /api/add-to-cart rejects a component unless an approved vendor has stock, so a
    catalog row without one gives the user a dead 'Add to Cart' button. Only surface
    components that can actually be purchased.
    """
    ...
    cursor.execute(
        f"""
        SELECT DISTINCT vc.component_id
        FROM vendor_components vc
        INNER JOIN vendors v ON v.id = vc.vendor_id
        WHERE v.approval_status = 'approved'
          AND vc.quantity >= 1
          AND vc.component_id IN ({placeholders})
        """,
        tuple(ids),
    )
    addable = {int(row[0]) for row in cursor.fetchall()}
    return [r for r in rows if int(r['id']) in addable]


def _catalog_matches_for_label(cursor, label: str, category: str) -> list[dict]:
    """Addable catalog rows for a detected part, most-relevant first, capped.

    Ranking: name mentions the part keyword first (robust to mis-categorised data),
    then the category's own stock/price ordering from _search_local_components.
    """
    rows = _search_local_components(
        cursor,
        category=category,
        limit=DETECTION_MATCH_LIMIT * 6,
        vendor_summary=True,
    )
    in_stock = [r for r in rows if int(r.get('stock') or 0) > 0]
    in_stock = _keep_addable(cursor, in_stock)  # only items the buyer can actually add

    keywords = DETECTION_NAME_KEYWORDS.get(label, ())
    if keywords:
        def _name_relevant(row) -> int:
            name = str(row.get('name') or '').lower()
            return 0 if any(kw in name for kw in keywords) else 1
        in_stock.sort(key=_name_relevant)  # stable: preserves stock/price order within group

    return in_stock[:DETECTION_MATCH_LIMIT]


def _attach_catalog_matches(detections: list[dict]) -> None:
    """Enrich each confirmed detection in place with `matches` (real DB components).

    Only in-stock items are attached (Live Catalog Resolution). Detection never
    fails because of the DB: on any error every detection just gets matches=[].
    """
    for det in detections:
        det.setdefault('matches', [])
        det.setdefault('match_count', 0)

    confirmed = [d for d in detections if d.get('type') != 'UNKNOWN']
    if not confirmed:
        return
    ...
    try:
        ...
        for det in confirmed:
            ...
            category = DETECTION_CATEGORY_MAP.get(label)
            if not category:
                continue
            if label not in cache:
                cache[label] = _catalog_matches_for_label(cursor, label, category)
            matches = cache[label]
            det['matches'] = matches
            det['match_count'] = len(matches)
            det['catalog_category'] = category
    except Exception:
        app.logger.exception('catalog match enrichment failed (detection still returned)')
    finally:
        if cursor:
            cursor.close()
        if connection and connection.is_connected():
...
def recommend_build():
    ...
    try:
        ...
        if intent == 'upgrade_request' and analysis.get('message'):
            return _recommend_build_ok(
                payload,
                markdown=analysis['message'],
...
def detect_component():
    ...
    try:
        ...
        # Live Catalog Resolution: attach real, in-stock components per detection.
