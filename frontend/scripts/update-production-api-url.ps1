...
$files = @(
    @{ Path = 'vercel.json'; Pattern = 'https://genspark-production\.up\.railway\.app'; Replace = $ApiUrl },
    @{ Path = '.env.production'; Pattern = 'VITE_API_BASE=.*'; Replace = "VITE_API_BASE=$ApiUrl" },
    @{ Path = 'src\config\deployUrls.js'; Pattern = "export const LIVE_API_URL = .*"; Replace = "export const LIVE_API_URL = '$ApiUrl';" }
)
