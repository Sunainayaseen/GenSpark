...
const AIChatbot = () => {
  ...
  // Lazy initializer: Date.now()/new Date() must not run on every render — only once,
  // for the very first render, which is exactly what useState(() => ...) guarantees.
  ...
  const getAIResponse = (userMessage) => {
    ...
    if (lowerMessage.includes('gaming') || lowerMessage.includes('game')) {
      ...
      // getAIResponse only ever runs from handleSend (a user-triggered event handler),
      // never during render, so Math.random() here doesn't violate render purity —
      // the compiler's static analysis can't see that call-only-from-handler invariant.
  ...
  // handleSend is a form-submit event handler — everything inside it (including the
  // setTimeout callback) runs outside render, so the Date.now() calls below don't
  // violate render purity even though the compiler's static check can't prove that.
  const handleSend = (e) => {
    e.preventDefault();
    ...
    setTimeout(() => {
      ...
      // eslint-disable-next-line react-hooks/purity -- random typing delay inside a setTimeout, not render
