...
const OrderStatus = () => {
  ...
  const priceBreakdown = useMemo(() => {
    if (!order) {
      return { itemsSubtotal: 0, shipping: 0, assembly: 0, total: 0 };
    }
    ...
    // Backend already computes this (get_order) — surfacing it here closes the gap
    // where total could exceed itemsSubtotal + shipping with no visible explanation.
    const assembly = Number(order.assembly_fee || 0);
    const total = Number(order.total_amount ?? itemsSubtotal + shipping + assembly);
    return { itemsSubtotal, shipping, assembly, total };
  }, [order]);
        ...
        <header className="os-hero">
          ...
          <div className="os-hero__facts">
            <div className="os-fact os-fact--accent">
              ...
              <span className="os-fact__value">PKR {priceBreakdown.total.toLocaleString('en-US')}</span>
            </div>
        ...
        <div className="order-content">
          <section className="order-card" aria-labelledby="order-summary-heading">
            <div className="order-header">
              ...
              <div className="order-price">PKR {priceBreakdown.total.toLocaleString('en-US')}</div>
            </div>
            {(order.items || []).length > 0 && (
                    ...
                    <li key={it.id ?? `${it.item_id}-${it.component_name}`}>
                      <div className="os-item-info">
                        ...
                        <span className="os-item-meta">
                          Qty {qty} · PKR {Math.round(unit).toLocaleString('en-US')} each
                        </span>
                      </div>
                      <span className="os-item-total">PKR {total.toLocaleString('en-US')}</span>
                    </li>
                  );
                })}
              </ul>
            ...
            <div className="order-price-breakdown" role="table" aria-label="Order pricing">
              <div className="order-price-row">
                ...
                <span>PKR {priceBreakdown.itemsSubtotal.toLocaleString('en-US')}</span>
              </div>
              <div className="order-price-row order-price-row--shipping">
                <span>Shipping (system)</span>
                <span>PKR {priceBreakdown.shipping.toLocaleString('en-US')}</span>
              </div>
              {priceBreakdown.assembly > 0 ? (
                <div className="order-price-row order-price-row--assembly">
                  <span>Assembly service</span>
                  <span>PKR {priceBreakdown.assembly.toLocaleString('en-US')}</span>
                </div>
              ) : null}
              <div className="order-price-row order-price-row--total">
                <span>Total</span>
                <span>PKR {priceBreakdown.total.toLocaleString('en-US')}</span>
              </div>
            </div>
                    ...
                    <article key={vo.id} className="os-vendor-card">
                      <header className="os-vendor-card__head">
                        ...
                        <span className="os-vendor-card__amount">
                          PKR {Number(vo.total_amount || 0).toLocaleString('en-US')}
                        </span>
                      </header>
