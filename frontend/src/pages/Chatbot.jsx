...
import { addBuildPartsBulk } from '../utils/buildPartMatcher';
...
const captureFrameFromVideo = (video, maxDim = 1280) => {
...
// _city: callers already pass the user's city, but build generation doesn't vary by
// city yet — kept in the signature so that wiring stays a one-line change later.
const generateBuilds = (purpose = 'Gaming', budget = 100000, _city = '') => {
...
const Chatbot = () => {
  ...
  const submitMessage = (rawInput) => {
    ...
    if (refine && parsedBudgetNum == null && lastBudgetNum > 0) {
      ...
      addBotMessage(
        `Refining your ${(purpose || 'Gaming').toLowerCase()} build into ${dir} option — new target ~PKR ${newBudget.toLocaleString('en-US')}.`
      );
      setTextInput('');
      setShowSuggestedBuilds(true);
      setIsTyping(true);
          ...
          {aiRecommendationMarkdown ? (
            <div className="preview-ai-build">
              <div className="preview-ai-build-scroll ai-recommendation-markdown ai-recommendation-markdown--enhanced">
                <BuildRecommendationCard markdown={aiRecommendationMarkdown} purpose={purpose} budget={budget ? `PKR ${Number(budget).toLocaleString('en-US')}` : ''} compatibility={aiCompatibility} editable />
              </div>
            ...
            <div className="preview-build">
              ...
              <p className="preview-price">
                <span className="preview-price-currency">PKR</span> {displayBuild.price?.toLocaleString('en-US')}
              </p>
