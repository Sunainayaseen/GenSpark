...
const BuildConfigurator = () => {
  ...
  const partsSubtotal = Number(summary?.total || 0);
  const finalTotal = partsSubtotal + ASSEMBLY_FEE + SHIPPING_FEE;
  const canAdd = !resolving && !adding && Number(summary?.addableCount || 0) > 0;
  ...
  const searchOptions = dbComponents.map((c) => ({
    id: c.id,
    name: c.name,
    brand: c.brand || '',
          ...
          {searchOptions.length === 0 ? (
            ...
            searchOptions.map((opt) => (
              <div key={opt.id} className="part-option">
                ...
                <div className="option-price">PKR {opt.price.toLocaleString()}</div>
              ...
              <div className="compatibility-status" role="status">
                ...
                {resolving ? (
                  ...
                  <span className={`cfg-chip ${stockChipClass}`}>
                    {summary.resolvedCount}/{slots.length} in stock
                  </span>
                ) : null}
              </div>
              ...
              <div className="price-breakdown">
                ...
                <div className="price-item">
                  ...
                  <span className="price-item-value">
                    {resolving ? 'Checking…' : `PKR ${partsSubtotal.toLocaleString()}`}
                  </span>
                </div>
                <div className="price-item">
                  <span className="price-item-label">Assembly</span>
                  <span className="price-item-value">PKR {ASSEMBLY_FEE.toLocaleString()}</span>
                </div>
                <div className="price-item">
                  <span className="price-item-label">Shipping (est.)</span>
                  <span className="price-item-value">PKR {SHIPPING_FEE.toLocaleString()}</span>
                </div>
                <div className="price-item total">
                  <span className="price-item-label">Estimated total</span>
                  <span className="price-item-value">
                    {resolving ? '—' : `PKR ${finalTotal.toLocaleString()}`}
                  </span>
                </div>
              </div>
                    ...
                    <li key={`${row.slot}-${row.label}`} className="config-build-component-row">
                      ...
                      {row.status === 'resolved' && Number(row.component?.price) > 0 ? (
                        <span className="config-build-component-price">
                          PKR {Number(row.component.price).toLocaleString()}
                        </span>
                      ) : (
