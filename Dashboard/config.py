...
class Config:
    ...
    # Hard cap on any single request body (proof-image uploads, /api/detect images) —
    # without this, an unbounded upload can exhaust memory before any route-level check runs.
    ...
    # Backend (Flask/Railway) — OAuth callbacks; verify-email uses build_verify_email_url()
    PREFERRED_URL = (
    ...
    # Hardcoded Railway host for registration verify-email (see build_verify_email_url)
    VERIFY_EMAIL_API_BASE = os.getenv(
        'VERIFY_EMAIL_API_BASE',
        'https://genspark-production.up.railway.app',
    ).strip()
...
class TestingConfig(Config):
    """In-memory SQLite + CSRF off so pytest runs without touching the real MySQL DB.

    create_app() detects TESTING and wires the in-memory engine (see app/__init__.py).
    """
    TESTING = True
    DEBUG = False
    WTF_CSRF_ENABLED = False
    MAIL_SERVER = ''  # never send real email during tests
