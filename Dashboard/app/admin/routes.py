...
from app.utils.dispatch import auto_assign_rider_for_order, assign_rider_to_order, riders_for_order
...
def inject_admin_notifications():
    """Feed the admin navbar bell with the logged-in admin's recent notifications
    (new orders, vendor proofs, rider pickup requests, new vendors). Injected into
    every admin-panel template."""
    if not current_user.is_authenticated or not current_user.is_admin:
        return {'admin_notifications': [], 'admin_unread_count': 0}
    try:
        ...
        db.session.rollback()
        rows, unread = [], 0
    return {'admin_notifications': rows, 'admin_unread_count': unread}
...
def notifications_read_all():
    """Mark all of the logged-in admin's notifications as read, then go back."""
...
def dashboard():
    ...
    # Revenue = value of fulfilled sales (orders that reached a completed/shipped state).
...
def order_detail(order_id):
    ...
    # Rider/delivery panel: which rider got the auto-assigned dispatch job.
    delivery = (
    ...
    if delivery:
        ...
        delivery_row = SimpleNamespace(
            ...
            rider_name=(rider_user.name if rider_user else '—'),
            rider_phone=(rider.phone if rider else None),
            rider_city=(rider.city if rider else None),
            rider_rating=(round(float(rider.rating or 0), 1) if rider else None),
            status=delivery.status,
            pickup_location=delivery.pickup_location,
            dropoff_location=delivery.dropoff_location,
            distance_km=delivery.distance_km,
            earning=float(delivery.earning or 0),
            created_at=delivery.created_at,
        )

    # Manual rider picker: offered whenever the order is past admin-review AND the
    # delivery (if any) hasn't been acted on yet — covers both "none assigned yet"
    # and "override the auto-pick before the rider starts" (delivery.status == 'assigned').
    rider_choices = []
    can_pick_rider = order.status not in ('pending', 'admin-review', 'rejected') and (
        not delivery or delivery.status == 'assigned'
    )
    if can_pick_rider:
        rider_choices = [
            SimpleNamespace(
                id=row['rider'].id,
                code=row['rider'].display_code,
                name=(row['rider'].user.name if row['rider'].user else '—'),
                city=row['rider'].city,
                rating=round(float(row['rider'].rating or 0), 1),
                distance_km=row['distance_km'],
                workload=row['workload'],
                is_online=row['is_online'],
            )
            for row in riders_for_order(order)
            if not delivery_row or row['rider'].id != delivery.rider_id
        ]
...
def admin_assign_rider(order_id):
    """Admin's manual override of the Auto-Assignment Bridge — picks a specific
    rider (e.g. nearest to the customer) instead of the best-score auto-pick."""
    order = Order.query.get_or_404(order_id)
    rider_id = request.form.get('rider_id', type=int)
    rider = Rider.query.get(rider_id) if rider_id else None
    if not rider:
        flash('Please choose a rider to assign.', 'warning')
        return redirect(url_for('admin.order_detail', order_id=order_id))

    result = assign_rider_to_order(order, rider)
    if result.get('created') or result.get('reassigned'):
        verb = 'reassigned' if result.get('reassigned') else 'assigned'
        flash(
            f'Order {verb} to {rider.display_code} '
            f'({rider.user.name if rider.user else "rider"}), '
            f'{result["distance_km"]} km from the customer.',
            'success',
        )
    elif result.get('reason') == 'same_rider':
        flash(f'{rider.display_code} is already assigned to this order.', 'info')
    elif result.get('reason') == 'in_progress':
        flash('Cannot reassign — the current rider has already started this delivery.', 'warning')
    else:
        flash('Could not assign that rider to this order.', 'warning')
    return redirect(url_for('admin.order_detail', order_id=order_id))
...
def reports():
    ...
    # Orders are multi-vendor (per line item) → count VendorOrder rows, not the master
    # Order (whose vendor_id is NULL for multi-vendor orders — see vendor.dashboard()).
    top_vendors = db.session.query(Vendor.shop_name, func.count(VendorOrder.id).label('cnt')).join(
        VendorOrder, VendorOrder.vendor_id == Vendor.id
    ).group_by(Vendor.id).order_by(func.count(VendorOrder.id).desc()).limit(10).all()
