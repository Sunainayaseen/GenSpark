...
def dashboard():
    ...
    assembly_orders = Order.query.filter_by(vendor_id=vendor.id).filter(Order.status.in_(['accepted', 'assembly'])).count()
    completed_orders = Order.query.filter_by(vendor_id=vendor.id).filter(Order.status.in_(['shipped', 'delivered'])).count()
    earnings = db.session.query(func.coalesce(func.sum(Payment.amount), 0)).join(
        Order, Order.id == Payment.order_id
    ).filter(Order.vendor_id == vendor.id, Payment.payment_status == 'completed').scalar() or 0
...
def earnings():
    vendor = get_vendor()
    payments = Payment.query.join(Order).filter(Order.vendor_id == vendor.id).order_by(Payment.created_at.desc()).all()
    total = sum(float(p.amount or 0) for p in payments if p.payment_status == 'completed')
    return render_template('vendor/earnings.html', vendor=vendor, payments=payments, total_earnings=total)
