...
def complete_checkout():
    ...
    # Stripe's StripeObject does not support dict-style .get() in this SDK version —
    # use getattr so a missing field returns None instead of raising AttributeError.
    ...
    try:
        ...
        # Assembly fee uses the same rule as COD checkout (CPU + motherboard).
        from app.services.fees import assembly_fee_for
        shipping_fee = max(0.0, shipping_fee)
        assembly_fee = assembly_fee_for(item_names)
        server_total = round(server_items_total + shipping_fee + assembly_fee, 2)
