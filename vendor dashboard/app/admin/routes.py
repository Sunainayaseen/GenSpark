...
def admin_approve_vendor_proof(order_id, vendor_order_id):
    ...
    db.session.commit()
    flash('Proof approved. Vendor can now dispatch this order from Shipment.', 'success')
...
def order_approve_generate(order_id):
    ...
    for vendor_id, order_items in vendor_buckets.items():
        ...
        db.session.add(Notification(
            ...
            title='Vendor order generated',
            message=f'Vendor order #{v_order.id} has been created.',
            type='order_update',
            related_id=v_order.id,
        ))
        generated_count += 1
